...
//if called accidently during/before init then exit
if (isnil "bis_fnc_init") exitwith {diag_log "HC_fnc_liveSpawnFromArray called before mission init!"};
		...
		if (_isAllowVeh && (random(1) < (_paramsPresence select 0)) && call compileFinal(_paramsPresence select 1)) then {
			...
			if (isNull _group) then {
				...
				if !((_paramsInitGr select 0) isEqualTo "") then {
					call compile format ["%1 = %2; publicvariable '%1'; _this = %2; %3",(_paramsInitGr select 0),_group,(_paramsInitGr select 1)];
				};
				
				if !((_paramsInitGr select 1) isEqualTo "") then {
					_group call compile (_paramsInitGr select 1);
				};
...

{
		...
		if !((_paramsInitV select 1) isEqualTo "") then {
			_obj call compile (_paramsInitV select 1); 
		};
