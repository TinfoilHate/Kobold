...
private ["_Unit", "_PZAI_MovedRecently", "_PZAI_VisuallyCanSee", "_NearestEnemy", "_intersections"];
	
	_Unit = _this select 0;
	_PZAI_MovedRecently = _this select 1;
	_PZAI_VisuallyCanSee = _this select 2;
	...
	//if (isNil "_NearestEnemy" || {_PZAI_MovedRecentlyCover} || {(typeName _NearestEnemy isEqualTo "ARRAY")} || {isNil "_Unit"} || {!(alive _NearestEnemy)} || {(_NearestEnemy distance _Unit) > 5000}) exitWith {};
	if (isNil "_NearestEnemy" || {(typeName _NearestEnemy isEqualTo "ARRAY")} || {isNil "_Unit"} || {!(alive _NearestEnemy)} || {(_DistanceCheck) > 2000}) exitWith {_Unit forcespeed -1;};
		...
		_ReturnedFriendly = [units (group _Unit),_Unit] call PZAI_fnc_ClosestObject;
	...
	if (_cansee > 0 && {(_DistanceCheck) < 25}) exitwith 
	{
			...
			_PZAI_VisuallyCanSee
			
	};
	

	if (_PZAI_MovedRecentlyCover) exitwith {};
	...
	if (_PZAI_MovedRecently) exitWith {};
