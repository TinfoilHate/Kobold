...
While {true} do {
	{
		...
		if (_group getvariable ["InitialWPSet",false]) then {
			if (alive _leader) then {
				...
				if (!(_trackername in PZAI_MARKERARRAY)) then {
					_markerlead = createMarker [_trackername,[0,0]];
					PZAI_MARKERARRAY pushback _trackername;
				...
				if (_usedest) then {
					...
					if (!(_destname in PZAI_MARKERARRAY)) then {
						_markerobj = createMarker[_destname,[0,0]];
						PZAI_MARKERARRAY pushback _destname;
					...
					if (_destname in PZAI_MARKERARRAY) then {
						...
						PZAI_MARKERARRAY = PZAI_MARKERARRAY - [_destname];
					};	
				};
				...
				if (_usetarget) then {
					...
					if (!(_targetname in PZAI_MARKERARRAY)) then {
						_markertarget = createMarker[_targetname,[0,0]];
						PZAI_MARKERARRAY pushback _targetname;
					...
					if (_trackername in PZAI_MARKERARRAY) then {
						...
						PZAI_MARKERARRAY = PZAI_MARKERARRAY - [_targetname];
					};
				};
			} else {
				_lastname = name _leader;
				...
				if (_trackername in PZAI_MARKERARRAY) then {
					...
					PZAI_MARKERARRAY = PZAI_MARKERARRAY - [_trackername];
				};
