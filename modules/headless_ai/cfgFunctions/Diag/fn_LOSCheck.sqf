...
//unit 1, unit 2
params ["_unit", ["_target", objnull, [objnull]], ["_dirCheck", true, [true]]];

if (_target isEqualTo objnull) exitwith {false};

if (_dirCheck && {!([_unit, _target] call FUNC(isFacing))}) exitwith {false};
...
private _eyeP2 = [_getposTarget select 0, _getposTarget select 1, (_getposTarget select 2) + (GVAR(AimTargetHeightAdjust))];
