 ...
 * 4: Optional location to fill with magazines ("" or blank is all locations) <STRING>
 *
 * Return Value:
 * Nil
 *
 * Example:
 * [_unit, _type, ["rhs_weap_akm", "rhs_30Rnd_762x39mm", "rhs_weap_ak74n", "rhs_30Rnd_545x39_AK"], 8, "vest"] call kobld_weaponHelper_fnc_addWeapon;
 *
 * Public: Yes
*/
...
params ["_unit", "_type", "_inputArray", "_max", ["_location", ""]];
...
private _accRandom = [
	(random 1 < (missionNamespace getVariable [("kobld_weaponHelper_opticChance_" + str(side _unit)),1])),
	(random 1 < (missionNamespace getVariable [("kobld_weaponHelper_muzzleChance_" + str(side _unit)),1])),
	(random 1 < (missionNamespace getVariable [("kobld_weaponHelper_pointerChance_" + str(side _unit)),1])),
	(random 1 < (missionNamespace getVariable [("kobld_weaponHelper_bipodChance_" + str(side _unit)),1]))
];
...
_itemsFinal apply {
	[_unit,_type,_x] call FNC_addItemOrg;
};

if (count _magazinesFinal > 0) then {
	_unit addPrimaryWeaponItem (selectRandom _magazinesFinal);
	_unit addHandgunItem (selectRandom _magazinesFinal);

	[_unit,_type,_magazinesFinal,_max,_location] call kobld_weaponHelper_fnc_addMagazine;
} else {
	[_unit,_type,_weaponFinal,_max,_location] call kobld_weaponHelper_fnc_addMagazine;
};
